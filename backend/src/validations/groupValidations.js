...
export const validateCreateGroup = [
  ...
  body('members.*.uuid').isUUID().withMessage('Each member must have a valid uuid.'),
  body('members.*.role')
    .optional()
    .isIn(['admin', 'member'])
...
export const validateUpdateGroup = [
  ...
  body('members.*.uuid').optional().isUUID().withMessage('Each member must have a valid uuid.'),
  body('members.*.role')
    .optional()
    .isIn(['admin', 'member'])
    .withMessage("Role must be either 'admin' or 'member'."),
  ...
  body('roleUpdates').optional().isArray().withMessage('roleUpdates must be an array.'),
